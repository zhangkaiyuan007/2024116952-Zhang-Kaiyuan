...
public:
    ImageROI(const vector<Point>& pts) {  // 修改为const引用以提高效率
        points = pts;
        if (points.empty()) {  // 检查points是否为空
...
private:
    ...
    static const int MAX_CACHE_SIZE = 100;  // 常量定义为静态
    
public:
    static Mat& getImage(const string& path) {
        if(cache.find(path) == cache.end()) {
            cache[path] = imread(path);  
            if (cache[path].empty()) {  // 检查图像是否读取成功
                throw runtime_error("Image load failed in cache");
            }
            // 实现缓存大小管理
            if (cache.size() > MAX_CACHE_SIZE) {
                cache.erase(cache.begin());  // 简单地移除最早加载的图像
            }
        }
        return cache[path];  
    }
    
    static void clearCache() {  // 静态成员函数
        cache.clear();  
    }
};
...
public:
    ImagePreprocessor(const Mat& img, const ProcessingConfig& cfg) : 
        srcImg(img.clone()), config(cfg) {}  // 深拷贝图像
...
public:
    ...
    void process() override {
        ...
        srcImg = processed;  // 直接赋值，无需手动分配内存
    }
};
...
public:
    ...
    void enhanceChannel(Mat& channel) {
        channel.convertTo(channel, CV_32F, enhancementFactor);  // 指定目标深度
    }
    
    void process() override {
        split(srcImg, channels);
        for(int i = 0; i < channels.size(); i++) {  // 修正循环条件
...
public:
    ImageProcessingPipeline(const string& imagePath) {
        ...
        if(originalImage.empty()) {
            throw runtime_error("Image load failed");  // 抛出标准异常
        }
    }
    ...
    ~ImageProcessingPipeline() {
        for(auto processor : processors) {  // 删除每个处理器对象
            delete processor;
        }
        processors.clear();
    }
...
int main() {
    try {
        ...
        // 添加处理器
        pipeline.addProcessor(new GeometricTransformer(pipeline.originalImage, config));
        pipeline.addProcessor(new ImageEnhancer(pipeline.originalImage, config));
        pipeline.addProcessor(new ImageFilter(pipeline.originalImage, config));
        pipeline.addProcessor(new FeatureExtractor(pipeline.originalImage, config));
